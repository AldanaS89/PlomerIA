...
def cargar():
    ...
    for p in plomeros_json:
        ...
        especialidad_mapeada  = mapear_especialidad(p["especialidad"])
        especialidad_original = p["especialidad"]

        # Si la especialidad no está en el mapa exacto, guardar el original en otra_especialidad
        key_original = especialidad_original.lower().strip()
        es_exacta    = key_original in ESPECIALIDAD_MAP
        otra         = especialidad_original if not es_exacta else None

        nuevo = Plomero(
            nombre            = nombre,
            apellido          = apellido,
            email             = email_ficticio,
            telefono          = f"11{p['id']:08d}",
            especialidad      = especialidad_mapeada,
            otra_especialidad = otra,
            ...
            total_trabajos    = 0,
            matricula_gas     = especialidad_mapeada == "GAS_MATRICULADO",
    ...
    print(f"\n🔑 Password de todos los plomeros: plomero1234")
